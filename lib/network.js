function renderNetwork(options) {
  ...
  if (options.use_simplified_tags) {
    ...
    options.tag_to_color = Object.fromEntries(Object.entries(options.tag_to_color).filter((entry) => distinctTags.has(entry[0])));
  }

  // update links to filter on dates if needed
  if (!options.use_animation && options.start_year != null && options.end_year != null) {
    options.links = options.links.filter(link => parseInt(link.to_date) >= options.start_year && parseInt(link.to_date) <= options.end_year)
  }

  let allEdges = options.links.map((link, i) => makeEdgeFromLink(link, i, options));
...
function activateCheckbox(state) {
  ...
  checkbox.addEventListener('change', (e) => {
    ...
    if (e.currentTarget.checked) {
      ...
      renderNetwork({ ...OFFICIALS_CONFIG, links: OFFICIALS_CONFIG.links.filter(e => e.to_date <= 1763) });
    }
  });
}
...
function makeEdgeFromLink(link, index, options) {
  ...
  return {
    ...
    middlePoint: options.bend_edges_via_metropole && link.via_metropole ? { x: 610, y: 170 } : null,
    color: (options.use_tags && link.tag in options.tag_to_color) ? options.tag_to_color[link.tag].color : (options.edge_color ?? '#009292')
  };
}
