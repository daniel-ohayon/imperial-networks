...
function prepare(entries) {
    ...
    for (entry of distinctOrderedByCount(entries, e => e.Author)) {
        ...
        authorNodes.push({
            ...
            y: (isLeft ? iLeft : iRight) * ROW_SPACING,
            color: '#c00000',
            kind: 'AUTHOR',
            size: 10,
            font: { color: 'white', size: AUTHOR_FONT_SIZE },
            type: 'def',
        });
    }
...
function enableModalOnNodeClick(network, dataById) {
    network.on("click", function(params) {
        ...
        const listHtml = connectedEdges
            .sort((e1, e2) => e1.rank - e2.rank)
            .map(e => `<li>${e.quote} <span class="citation">– ${e.book}</citation></li>`).join('\n');
