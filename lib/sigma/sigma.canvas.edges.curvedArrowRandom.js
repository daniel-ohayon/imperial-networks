function mulberry32(a) {
  ...
  // We seed the RNG with each edge's ID so that the graph doesn't change 
  // each time we reload the page
  return function () {
    var t = a += 0x6D2B79F5;
...
getRandomQuadraticControlPoint = function (seed, x1, y1, x2, y2) {
  ...
  return {
    x: (x1 + x2) / 2 + p * (y2 - y1) / 4,
    y: p * (y1 + y2) / 2 + p * sgn * (x1 - x2) / 4
  };
};

function getControlPointForFixedMidpoint(seed, sX, sY, mX, mY, tX, tY) {
  // Coordinates of the control point for a curve passing through 3 given points:
  // https://stackoverflow.com/questions/6711707/draw-a-quadratic-b%C3%A9zier-curve-through-three-given-points
  // We randomize the midpoint a little bit too so that the edges don't all intersect at the same point
  numericSeed = xfnv1a(seed)();
  seededRNG = mulberry32(numericSeed);
  pX = seededRNG() * 40 - 20;
  pY = seededRNG() * 40 - 20;
  ...
  sigma.canvas.edges.curvedArrowRandom =
    function (edge, source, target, context, settings) {
      var color = edge.color,
        ...
        tX = target[prefix + 'x'],
        tY = target[prefix + 'y'],
        aSize = settings('hideArrows') || edge.hideArrow ? 0 : Math.max(size * 2.5, settings('minArrowSize')),
        d,
        aX,
        aY,
        vX,
      ...
      if (edge.middlePoint != null && source.id !== target.id) {
        const mp = edge.middlePoint;
        cp = getControlPointForFixedMidpoint(edge.id, sX, sY, mp.x, mp.y, tX, tY);
      } else {
